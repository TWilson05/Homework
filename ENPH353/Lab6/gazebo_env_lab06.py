...
class Gazebo_Lab06_Env(gazebo_env.GazeboEnv):
    ...
    def process_image(self, data):
        ...
        maxHeight = cv_image.shape[0]
        maxWidth = cv_image.shape[1]
        lookHeight = int(0.90*maxHeight)

        line = []
        for x in range (0,maxWidth):
            if img_bin[lookHeight,x] == 0:
                line.append(x)
        if len(line) == 0:
            self.timeout += 1
            avg = maxWidth//2
        ...
        # see which state the robot is in
        index = int(avg/(maxWidth/10))
        state[index] = 1

                
        cv2.putText(img=cv_image, text=str(state), org=(20, 20), fontFace=cv2.FONT_HERSHEY_TRIPLEX, fontScale=0.5, color=(0, 255, 0),thickness=1)
